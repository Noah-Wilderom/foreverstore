...
func defaultRootFolderName() string {
...
var DefaultPathTranformFunc = func(key string) PathKey {
	return PathKey{
		PathName: key,
		Filename: key,
	}
...
func (s *Store) pathWithRoot(path ...string) string {
	return fmt.Sprintf("%s/%s", s.Root, strings.Join(path, "/"))
}

func (s *Store) Has(id string, key string) bool {
	pathKey := s.PathTransformFunc(key)
	fullPathWithRoot := s.pathWithRoot(id, pathKey.FullPath())
...
func (s *Store) Delete(id, key string) error {
	...
	firstPathNameWithRoot := s.pathWithRoot(id, pathKey.FirstPathName())
...
func (s *Store) openFileForWriting(id string, key string) (*os.File, error) {
	pathKey := s.PathTransformFunc(key)
	pathNameWithRoot := s.pathWithRoot(id, pathKey.PathName)
